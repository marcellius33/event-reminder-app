...
class FilterComposer
{
    ...
    private static function event(string $name, ?string $value, ?string $default): array
    {
        return [
            'name'      => $name,
            'label'     => self::getLabel($name),
            ...
            'behaviour' => self::Event,
            'default'   => $default,
            'value'     => $value,
        ];
    }

    private static function getOptionNameByValue(array $options, ?string $value): ?string
    {
        return collect($options)->firstWhere('value', $value)['name'] ?? null;
    }
    ...
    private static function getMethodName(AllowedFilter $allowedFilter): string
    {
        ...
        if (($filterClass instanceof FiltersEvent)) {
            return 'event';
        } else if (($filterClass instanceof FiltersPartial)) {
            return 'partialText';
        } else {
            return 'exactText';
        }
